...
Read more about [configuring the return page](https://docs.stripe.com/payments/accept-a-payment.md?platform=web&ui=embedded-form#return-page) and other options for [customising redirect behaviour](https://docs.stripe.com/payments/checkout/custom-success-page.md?payment-ui=embedded-form).
...
![The Payment Element and a saved payment method tickbox](https://b.stripecdn.com/docs-statics-srv/assets/spm-save.fe0b24afd0f0a06e0cf4eecb0ce2403a.png)
...
![Authenticate or enrol with Link directly in the Payment Element during checkout](https://b.stripecdn.com/docs-statics-srv/assets/link-in-pe.2efb5138a4708b781b8a913ebddd9aba.png)
...
> #### Changes for Accounts v2
> 
> If your integration uses Accounts v2, then use the `customer_account` parameter instead of the `customer` parameter. Where the code example sets the `customer` parameter to the ID of the Customer, instead set the `customer_account` parameter to the ID of the Account.
> 
> The Account must have the `customer` configuration.
...
In your app’s checkout screen, fetch the PaymentIntent client secret, CustomerSession client secret, Customer ID, and publishable key from the endpoint you created in the previous step. Use `StripeAPI.shared` to set your publishable key and initialise the [PaymentSheet](https://stripe.dev/stripe-ios/stripe-paymentsheet/Classes/PaymentSheet.html).
...
curl https://api.stripe.com/v1/ephemeral_keys \
  ...
  -H "Stripe-Account: 2025-08-27.basil" \
  -X "POST" \
  -d "customer"="{{CUSTOMER_ID}}" \
...
post '/payment-sheet' do
  ...
  ephemeralKey = Stripe::EphemeralKey.create({
    customer: customer['id'],
  }, {stripe_version: '2025-08-27.basil', stripe_account: '{{CONNECTED_ACCOUNT_ID}}'})
  paymentIntent = Stripe::PaymentIntent.create({
    amount: 1099,
    currency: 'eur',
  ...
  (request, response) -> {
    ...
    EphemeralKeyCreateParams ephemeralKeyParams =
      EphemeralKeyCreateParams.builder()
        ...
        .setStripeVersion("2025-08-27.basil")
        .setCustomer(customer.getId())
        .build();
...
func handlePaymentSheet(w http.ResponseWriter, r *http.Request) {
  ...
  ekparams := &stripe.EphemeralKeyParams{
    ...
    StripeVersion: stripe.String("2025-08-27.basil"),
  }
