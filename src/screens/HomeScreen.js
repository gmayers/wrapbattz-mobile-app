...
// Modals
import NFCModal from './home/components/NFCManager/NFCModal';
import AssignDeviceModal from './home/components/AssignDevice/AssignModal';
import ReturnDeviceModal from './home/components/ReturnDevice/ReturnModal';

// Hooks and Context
import { useAuth } from '../context/AuthContext';
import useDevices from './home/hooks/useDevices';
import useLocations from './home/hooks/useLocations';
...
const HomeScreen = ({ navigation }) => {
  ...
  // Effects
  useEffect(() => {
    // Add navigation listener to set header options
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.buttonContainer}>
          <Button
            title="NFC Manager"
            onPress={() => toggleModal('nfc', true)}
            size="small"
            style={styles.headerButton}
          />
          <Button
            title="Assign Device"
            onPress={() => toggleModal('assign', true)}
            size="small"
            style={styles.headerButton}
          />
        </View>
              ...
              {devices.length === 0 ? (
                ...
                <>
                  ...
                  {devices.length > 5 && (
                    <TouchableOpacity
                      style={styles.viewAllButton}
                      onPress={() => navigation.navigate('AllDevices', { devices })}
                    >
                      <Text style={styles.viewAllText}>
      ...
      {/* Modals */}
      <NFCModal 
        visible={modalVisibility.nfc}
        onClose={() => toggleModal('nfc', false)}
      />

      <AssignDeviceModal
        visible={modalVisibility.assign}
      ...
      <ReturnDeviceModal
        visible={modalVisibility.return}
        device={selectedReturnDevice}
        locations={locations}
        onClose={() => {
          toggleModal('return', false);
